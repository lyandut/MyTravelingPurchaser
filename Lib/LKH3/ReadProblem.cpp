 ...
 * The number of etxernal salesmen in an OCMTSP.
 *
 * GRID_SIZE : <real>
...
static void Read_EXTERNAL_SALESMEN(void);
...
static void Read_EXTERNAL_SALESMEN()
{
    char *Token = strtok(0, Delimiters);

    if (!Token ||
        (ExternalSalesmen == 0 && !sscanf(Token, "%d", &ExternalSalesmen)))
        eprintf("EXTERNAL_SALESMEN: Integer expected");
    if (Salesmen < 0)
        eprintf("EXTERNAL_SALESMEN: < 0");
}
