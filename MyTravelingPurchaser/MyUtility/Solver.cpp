...
int Solver::Cli::run(int argc, char * argv[]) {
	...
	Problem::Input input = TPPLIBInstanceBuilder::buildFromFile(env.instPath);
    if (!input) { return -1; }
...
bool Solver::solve() {
    ...
    for (int i = 0; i < workerNum; ++i) {
        ...
        Log(LogSwitch::Szx::Framework) << "worker " << i << " got " << solutions[i].opitimization << endl;
        if (solutions[i].opitimization <= bestValue) { continue; }
        bestIndex = i;
        bestValue = solutions[i].opitimization;
    }
...
void Solver::record() const {
    ...
    CsvReader cr;
    ifstream ifs(Environment::DefaultInstanceDir() + "Baseline_" + INSTANCESET + ".csv");
    if (!ifs.is_open()) { return; }
    const List<CsvReader::Row> &rows(cr.scan(ifs));
    ifs.close();
    String Lambda, Dimension, DemandNum, MarketNum, Opt, Sec;
    for (auto r = rows.begin(); r != rows.end(); ++r) {
        if (env.friendlyInstName() != r->front()) { continue; }
		if (INSTANCESET == "CapEuclideo") {
			Lambda = (*r)[1];
			Dimension = (*r)[2];
			DemandNum = (*r)[3];
			MarketNum = (*r)[4];
	...
	log << env.friendlyLocalTime() << ","
		...
		<< env.instPath << ","
		<< feasible << ","
		<< (output.opitimization - stod(Opt)) / stod(Opt) << ","
		<< output.opitimization << "," 
		<< Opt << "," 
		<< output.tour.size() - 1 << ","
		<< MarketNum << ","
    ...
    if (logFile.tellp() <= 0) {
        logFile << "Time,ID,Instance,Feasible,Compare,MyCost,OptCost,MyMarketNum,OptMarketNum,Duration,RefDuration,PhysMem,VirtMem,RandSeed,Config,Generation,Iteration,Solution" << endl;
    }
    logFile << log.str();
    logFile.close();
    #endif // SZX_DEBUG
...
bool Solver::check(const Solution &checkerObj) const {
	return MyTPPChecker::simulateCheck(checkerObj, input);
}
...
bool Solver::optimize(Solution &sln, ID workerId) {
	...
	SolverDispatcher::iteratedModel(sln, input, env);
